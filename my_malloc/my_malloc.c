...
// FF: search the first enough node to free
// arrange the node structure inside and set properties
// return the first fitting node, if not found, return null  
struct link_node * ff_search_free_node(size_t size){
  ...
  while(temp != NULL){
    if(temp->isfree == 1 && temp->length >= size){      
      ...
      total_free -= size;
      // /* // split the memory into two portions if space is enough
      if (temp->length - size > NODE_SIZE){
	split(temp, size);
      }
      // */
      return temp; 
...
// main api to malloc memory in three senarios problems
// decide if we should use left memory (after precious sbrk's)
// or use sbrk to utilize new momery chunk
// return the content as the API return value for our customer (void*, represent any type of data)
...
// completely same as free funciton with FF strategy
