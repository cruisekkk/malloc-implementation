...
int main(int argc, char *argv[])
{
  ...
  unsigned long largest_allocatable_memory_blcok;
  ...
  largest_allocatable_memory_blcok = get_largest_free_data_segment_size();
  printf(" total_size_of_free_memory = %lu,  largest_allocatable_memory_blcok = %lu\n",  total_size_of_free_memory,  largest_allocatable_memory_blcok);

  double elapsed_ns = calc_time(start_time, end_time);
  printf("Execution Time = %f seconds\n", elapsed_ns / 1e9);
  printf("Fragmentation  = %f\n", 1 - ((float) largest_allocatable_memory_blcok/(float) total_size_of_free_memory));
